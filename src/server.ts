...
async function main(): Promise<void> {
    ...
    io.on("connection", (socket: any) => {
        ...
        socket.on("playerLogin", async (room: string, callback: any) => {
            const roomPlayer = (await RoomManager.rooms[room].getPlayerList()).length;
            if (roomPlayer >= 4) {
        ...
        socket.on("startGame", (room: string, callback: any) => {
            ...
            const numPlayer = RoomManager.rooms[room].numPlayer; // players = seluruh player yang ada
            if (numPlayer === 4) {
                io.to(room).emit("initState", room);
                RoomManager.rooms[room].Init();
                return callback(RoomManager.rooms[room].getPlayerNameList()); // player names
            }
            callback();
        });

        socket.on("requestCard", async (id: string, room: string) => {
            const playerHand = RoomManager.rooms[room].players[id].Hand;
            socket.emit("dealCard", playerHand.toStringArray()); // kasih kartu
            // players.updatePlayerHand(id, playerHand); // update player hand
            // console.log(JSON.stringify(rooms, undefined, 2));

            const defaultChangeCard = await RoomManager.rooms[room].players[id].defaultChangeCard();
            RoomManager.rooms[room].changedTiles[id] = defaultChangeCard;
            socket.emit("changeCard", defaultChangeCard.toStringArray(), 20000);
        });
